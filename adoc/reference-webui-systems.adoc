...
If you click menu:Systems[Overview] on the left bar, an overview listing of your menu:Systems[] appears.
...
The menu:Systems[] page displays a list of all your registered systems.
...
identifies package updates for the system, but the package updater (such as Red Hat Update Agent or {yast}) responds with a message like "Your system is fully updated", a conflict likely exists in the system's package profile or in the [path]``up2date``
 configuration file.
...
Non-compliant systems have packages installed which are not available from {susemgr}. menu:Packages[] shows how many installed packages are not available in the channels assigned to the system.
...
The menu:Without System Type[] page displays systems without System Type.
...
You can filter duplicate entitlements by menu:IP Address[], menu:IPv6 Address[], menu:Hostname[], or menu:MAC address[] by clicking the respective tab.
...
NOTE: If updates are available for any entitled system, the message menu:Software Updates Available[] appears, displaying the number of critical and non-critical updates and the sum of affected packages.
To apply these updates, click menu:Packages[] and select some or all packages to update, then click menu:Upgrade Packages[].
+



.menu:System Info[]Hostname::
...
To prevent the application of automated patch updates, deselect menu:Auto Patch Update[]from the menu:System
...
+
In addition to the configuration of {susemgr}, to receive pushed actions each client system must have the [systemitem]``osad``
...
Clicking the menu:Edit These Properties[] link right beside the menu:System Properties[] title opens the menu:System Details[Properties]
 subtab.
...
The next time the {susemgr} Daemon ([systemitem]``rhnsd``
...
When done click menu:Next[] to schedule the Software Channel Change action.
...
The menu:Provisioning[] tab and its subtabs allow you to schedule and monitor {ay} or Kickstart installations and to restore a system to its previous state.

.Available for Clients Using the "`Traditional`"Method
[NOTE]
====
The note menu:Provisioning[] tab will be available when adding a client using the "`traditional`"
 method (system type ``management``). Using Salt the menu:Provisioning[] tab will not be available (system type ``salt``).
====
...
If errors prevent correct execution, a brief message with an explanation will be displayed in the menu:Note[] column.
...
The menu:Snapshots[] subtab lists all snapshots for the system, including the reason the snapshot was taken, the time it was taken, and the number of tags applied to each snapshot.
...
To revert to a previous configuration, click the menu:Reason[] for the snapshot and review the potential changes on the provided subtabs, starting with menu:Rollback[].
...
When satisfied with the reversion, return to the menu:Rollback[] subtab and click the menu:Rollback to Snapshot[] button.
...
To tag the most recent snapshot, click menu:Create System Tag[], enter a descriptive term in the menu:Tag name[] field, and click the menu:Tag Current Snapshot[] button.
You may then revert using this tag directly by clicking its name in the Snapshot Tags list.
To delete tags, select their check boxes, click menu:Remove Tags[], and confirm the action.
...
The menu:Virtualization[] tab has two subtabs, menu:Details[] and menu:Kickstart[].
...
.
Then with the drop-down box activate menu:Unmanaged[], menu:Installed[], or menu:Removed[].
...
States which have been created on the menu:States Catalog[] page located under menu:Salt[] on the left bar may be assigned to a system on the menu:States[Custom]
 page.
...
To unschedule a pending event, select the event and click the menu:Cancel Events[] button at the bottom of the page.
...
The menu:System Groups[] page allows {susemgr} users to view the menu:System Groups[] list.
...
To change this information, click menu:Edit These Properties[], make your changes in the appropriate fields, and click the menu:Update Group[] button.
...
Select the check box next to all packages to be verified, then click the menu:Verify Packages[] button.
...
If you enable menu:with recommended[], recommended child channels are automatically selected for subscription.
+
...
Newer versions created after scheduling are disregarded.
Find the results in the main menu:Schedule[] category or within the menu:System Details[Events]
 tab.
...
If you use menu:Autoinstall by IP Address[], {susemgr} will automatically pick a profile for each system so that the system's IP address will be in one of the IP ranges specified in the profile itself.
...
With PXE boot, you cannot only reinstall clients, but automatically install machines that do not have an operating system installed yet. {susemgr} and its network must be properly configured to enable boot using PXE.
...
Click menu:Set a custom value for selected systems[], then the name of a key.
Enter values for all selected systems, then click the menu:Set Values[] button.
To remove values for all selected systems, click menu:Remove a custom value from selected systems[], then the name of the key.
...
Listed under menu:Systems[Visualization]
 you will find the menu:Virtualization Hierarchy[], menu:Proxy Hierarchy[], and menu:Systems Grouping[] subpages.
...
The Activity selections (menu:Days Since Last Check-in[], for example) are useful in finding and removing outdated system profiles.

Type the keyword, select the criterion to search by, use the radio buttons to specify whether you want to query all systems or only those in the menu:System Set Manager[], and click the menu:Search[] button.
...
+
Selecting `{susemgr}
Default` allows client systems to register with the {suse}-provided default channel that corresponds with their installed version of {sle}.
...
To change the settings of a key, click the key's description in the list to display its menu:Details[] page (see <<_fig_s1_sm_systems_akey_details>>). Via additional tabs you can select child channels, packages, configuration channels, group membership and view activated systems.
...
Separate keys with a comma at the command line with [command]``rhnreg_ks`` or in a {kickstart} profile in the menu:Activation Keys[] tab of the menu:Autoinstallation Details[] page.
...
Enter a menu:Profile Name[] and menu:Profile Description[], then click menu:Create Profile[].
...
Enter a menu:Label[], menu:Hostname[], menu:Port[], menu:Username[] and menu:Password[].
