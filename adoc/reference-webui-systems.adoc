...
== Overview
(((|000susemgr; Administrator,System Overview)))
...
== Systems
(((system list)))
(((|000susemgr; Administrator,viewing a list of)))
(((|000susemgr; Administrator,status)))
...
-- System is being deployed using |000ay; or Kickstart. 
+ 
...
* menu:Packages[] : Total number of package updates for the system, including packages related to patch alerts and newer versions of packages not related to patch alerts. For example, if a client system that has an earlier version of a package installed gets subscribed to the appropriate base channel (such as |000sle; 12 SP2), that channel may have an updated version of the package. If so, the package appears in the list of available package updates. 
+

.Package Conflict
[IMPORTANT]
====
If |000susemgr; identifies package updates for the system, but the package updater (such as Red Hat Update Agent or |000yast;) responds with a message like |000quot;Your system is fully updated|000quot;, a conflict likely exists in the system's package profile or in the [path]_up2date_
 configuration file.
...
You have permission if you are the only user in your organization, if you are a |000susemgr; Administrator, or if the system belongs to a group for which you have admin rights. 
...
To reach this page, select menu:Systems[Systems > Physical Systems]
 from the left bar.
This page lists each physical system of which |000susemgr; is aware. 
...
To reach this page, select menu:Systems[Systems > Virtual Systems]
 from the left bar.
This page lists each virtual host of which |000susemgr; is aware and the guest systems on those hosts. 
...
Only guests registered with |000susemgr; are displayed with blue text.
...
Non-compliant systems have packages installed which are not available from |000susemgr;. menu:Packages[]
...
The menu:Inactive Systems[]
 page displays systems that have not checked in with |000susemgr; for 24 hours or more. 

.
image::systems_inactive.png[]


Checking in means that Zypper on |000sle; or Red Hat Update Agent on |000rhel; client systems connects to |000susemgr; to see if there are any updates available or if any actions have been scheduled.
If you see a message telling you that check-ins are not taking place, the client system is not successfully connecting to |000susemgr;. The reason may be one of the following: 

* The system is not entitled to any |000susemgr; service. System profiles that remain unentitled for 180 days (6 months) are removed. 
* The system is entitled, but the |000susemgr; daemon ([path]_rhnsd_ ) has been disabled on the system. Refer to <<_bp.contact.methods.rhnsd>> for instructions on restarting and troubleshooting. 
...
* The system is connected to a |000susemgr; Proxy Server or |000susemgr; that has not been properly configured. 
* The system itself has not been properly configured, perhaps pointing at the wrong |000susemgr; Server. 
* The system is not in the network. 
* Some other barrier exists between the system and the |000susemgr; Server. 
...
The menu:Proxy[]
 page displays the |000susemgrproxy; Server systems registered with your |000susemgr; server. 
...
Active entitlements are in gray, while inactive entitlements are highlighted in yellow and their check boxes checked by default for you to delete them as needed by clicking the menu:Delete Selected[]
 button.
Entitlements are inactive if the system has not checked in with |000susemgr; in a time specified via the drop-down box menu:A system
...
The report can be used to prioritize maintenance actions on the systems registered to |000susemgr;. 
...
System Types define the set of functionalities available for each system in |000susemgr; such as the ability of installing software or creating guest virtual machines. 
...
When systems are registered to |000susemgr;, they are displayed on the menu:Systems[Overview]
 page.
...
It does, however, remove your ability to manage it via |000susemgr;. 

If you mistakenly deleted a system profile from |000susemgr;, you may re-register the system using the bootstrap script or `rhnreg_ks` manually. 
====
...
(((deleting a system)))
(((|000susemgr; Administrator,Details)))
...
==== System Details |000gt;Details|000gt;Overview


This system summary page displays the system status message and the following key information about the system: 

.menu:System Status[]::
This message indicates the current state of your system in relation to |000susemgr;. 
+
...
|000susemgr; System ID::
A unique identifier generated each time a system registers with |000susemgr;. 
+


[NOTE]
====
The system ID can be used to eliminate duplicate profiles from |000susemgr;. Compare the system ID listed on this page with the information stored on the client system in the [path]_/etc/sysconfig/rhn/systemid_
 file.
...
For example, if a user logs in to the client directly and runs |000yast; Online Update (on |000slea;) or `pup` (on |000rhela;), the update tool will install available patches even if the system is locked in the Web interface. 
...
An example is the menu:|000susemgr; Tools[]
channel. 

.menu:System Events[]
Checked In::
The date and time at which the system last checked in with |000susemgr;. 

Registered::
The date and time at which the system registered with |000susemgr; and created this profile. 
...
When the client checks in after the scheduled start time, |000susemgr; will instruct the system to restart itself. 
====
...
+
The so-called OSA status is also displayed for client systems registered with |000susemgr; that have the OSA dispatcher (osad) configured. 
+
Push enables |000susemgr; customers to immediately initiate tasks rather than wait for those systems to check in with |000susemgr;. Scheduling actions through push is identical to the process of scheduling any other action, except that the task can immediately be carried out instead of waiting the set interval for the system to check in. 
+
In addition to the configuration of |000susemgr;, to receive pushed actions each client system must have the [path]_osad_
...
==== System Details |000gt;Details|000gt;Properties
...
The |000susemgr; Daemon ([path]_rhnsd_
...
==== System Details |000gt;Details|000gt;Remote Command
...
. On |000slea; clients, subscribe the system to the |000susemgr; Tools child channel. Then use Zypper to install the ``rhncfg``, ``rhncfg-client``, and `rhncfg-actions` packages, if not already installed: 
+
...
----
+
On |000rhela; clients, subscribe the system to the Tools child channel.
...
----
. Log in to the system as root and add the following file to the local |000susemgr; configuration directory: [path]_allowed-actions/scripts/run_ . 
+
...
----
** Create an empty [path]_run_ file in that directory to act as a flag to |000susemgr;, signaling permission to allow remote commands: 
+
...
==== System Details |000gt;Details|000gt;Reactivation[Management]
(((reactivating,systems)))


Reactivation keys include this system's ID, history, groups, and channels.
This key can then be used only once with the `rhnreg_ks` command line utility to re-register this system and regain all |000susemgr; settings.
...
[WARNING]
====
When autoinstalling a system with its existing |000susemgr; profile, the profile uses the system-specific activation key created here to re-register the system and return its other |000susemgr; settings.
...
==== System Details |000gt;Details|000gt;Hardware
...
If the hardware profile looks incomplete or outdated, click the menu:Schedule Hardware
     Refresh[]
 button.
The next time the |000susemgr; Daemon ([path]_rhnsd_
) connects to |000susemgr;, it will update your system profile with the latest hardware information. 

[[_s5_sm_system_details_migrate]]
==== System Details |000gt;Details|000gt;Migrate
...
==== System Details |000gt;Details|000gt;Notes
...
==== System Details |000gt;Details|000gt;Custom Info
...
==== System Details |000gt;Details|000gt;Proxy[Proxy]


This tab is only available for |000susemgrproxy; systems.
The tab lists all clients registered with the selected |000susemgrproxy; server. 

[[_s4_sm_system_details_channels]]
=== System Details |000gt;Software
...
==== System Details|000gt;Software|000gt;Patches
...
==== System Details |000gt;Software|000gt;Packages
...
[WARNING]
====
When new packages or updates are installed on the client via |000susemgr;, any licenses (EULAs) requiring agreement before installation are automatically accepted. 
====
...
The next time the |000susemgr; daemon ([path]_rhnsd_
) connects to |000susemgr;, it updates your system profile with the latest list of installed packages. 
...
==== System Details |000gt;Software|000gt;Software Channels
...
==== System Details |000gt;Software|000gt;SP Migration
...
[WARNING]
====
During migration |000susemgr; automatically accepts any required licenses (EULAs) before installation. 
====


Beginning with SLE 12 |000suse; supports service pack skipping, it is now possible to migrate from for example, SLE 12 to SLE 12 SP2.
...
=== System Details |000gt;Configuration[Management]
...
==== System Details|000gt;Configuration|000gt;Overview
...
==== System Details |000gt;Configuration|000gt;View/Modify Files
...
==== System Details |000gt;Configuration|000gt;Add Files
...
enter the full path of any files you want import into |000susemgr; or select deployable configuration files from the menu:Import Existing
Files[]
list.
...
==== System Details |000gt;Configuration|000gt;Deploy Files
...
==== System Details |000gt;Configuration|000gt;Compare Files

##emap 2014-04-22: Still accurate?#
 This subtab compares a configuration file stored on the |000susemgr; with the file stored on the client.
...
==== System Details|000gt;Configuration|000gt;Manage Configuration Channels
...
=== System Details |000gt;Provisioning[Management]


The menu:Provisioning[]
 tab and its subtabs allow you to schedule and monitor |000ay; or Kickstart installations and to restore a system to its previous state. 

|000ay; is a |000sle; and Kickstart is a Red Hat utility|000mdash;both allow you to automate the reinstallation of a system.
...
==== System Details|000gt;Provisioning|000gt;Autoinstallation
...
In this case |000susemgr; and its network must be properly configured to allow boot using PXE. 
...
For example, to |000kickstart; a system that joins the network of a specific organization (for example the Engineering department) you can create a profile variable to set the IP address and the gateway server address to a variable that any system using that profile will use.
...
For example, the [option]``network`` portion of a |000kickstart; file could look like the following: 
...
[NOTE]
====
There is a hierarchy when creating and using variables in |000kickstart; files.
System |000kickstart; variables take precedence over profile variables, which in turn take precedence over distribution variables.
...
For more information about Cobbler and |000kickstart; templates, refer to <<_advanced.topics.cobbler>>
. 

[[_s5_sdc_provisioning_powermgnt]]
==== System Details|000gt;Provisioning|000gt;Power Management


|000susemgr; allows you to power on, off, and reboot systems via the IPMI protocol if the systems are IPMI-enabled. 

.
image::system_details_traditional_provisioning_power_management.png[]


You need a fully patched |000susemgr; installation.
To use any power management functionality, IPMI configuration details must be added to |000susemgr;. First select the target system on the systems list, then select menu:Provisioning[Power
     Management]
.
...
Those are not supported by |000susemgr; but can be used by adding the fence agent names as a comma-separated list to the `java.power_management.types` configuration parameter. 

[[_s4_sm_system_details_shots]]
==== System Details|000gt;Provisioning|000gt;Snapshots


Snapshots enable you to roll back the system's package profile, configuration files, and |000susemgr; settings. 
...
[NOTE]
====
There is no maximum number of snapshots that |000susemgr; will keep, thus related database tables will grow with system count, package count, channel count, and the number of configuration changes over time.
...
==== System Details |000gt;Provisioning|000gt;Snapshot Tags
...
=== System Details |000gt;Virtualization
...
==== System Details |000gt; Virtualization|000gt; Details
...
[NOTE]
====
If a guest system has not registered with |000susemgr;, this information appears as plain text in the table. 
====
...
=== System Details |000gt;Groups
(((|000susemgr; Administrator,assigning and removing)))
...
==== System Details|000gt;Groups|000gt;List/Leave
...
Only System Group Administrators and |000susemgr; Administrators can remove systems from groups.
...
==== System Details |000gt;Groups|000gt;Join
...
Only System Group Administrators and |000susemgr; Administrators can add a system to groups.
Non-admins see a menu:Review this system's group
     membership[]
 page.
...
=== System Details |000gt;Audit[Management]
...
=== System Details |000gt;States[Salt]
(((system states)))
(((|000susemgr; Administrator,System states)))
...
==== System Details |000gt;States|000gt;Packages
...
==== System Details |000gt;States|000gt;Custom
...
==== System Details |000gt;States|000gt;Highstate
...
=== System Details |000gt;Formulas[Salt]
...
==== System Details|000gt;Events|000gt;Pending
...
==== System Details |000gt;Events|000gt;History
...
== System Groups
(((system group)))
(((|000susemgr; Administrator,list of)))
(((|000susemgr; Administrator,System Group List)))
(((Web Interface,System Groups)))


The menu:System Groups[]
 page allows |000susemgr; users to view the menu:System Groups[]
 list. 
...
Only System Group Administrators and |000susemgr; Administrators may perform the following additional tasks: 
...
(((|000susemgr; Administrator,adding and removing)))
...
==== System Group Details|000gt;Details
(((|000susemgr; Administrator,deleting)))
...
==== System Group Details |000gt;Systems
...
Clicking it does not delete systems from |000susemgr; entirely.
...
==== System Group Details |000gt;Target Systems
...
==== System Group Details |000gt;Patches
...
==== System Group Details |000gt;Admins


List of all organization users that have permission to manage the system group.
|000susemgr; Administrators are clearly identified.
...
==== System Group Details |000gt;States[Salt]
...
[NOTE]
====
States are applied according to the following order of hierarchy within |000susemgr;: 
...
=== System Set Manager |000gt;Overview
...
=== System Set Manager |000gt;Systems
...
=== System Set Manager |000gt;Patches
...
=== System Set Manager |000gt;Packages
...
==== System Set Manager |000gt;Packages|000gt;Install
...
==== System Set Manager |000gt;Packages|000gt;Remove
...
==== System Set Manager |000gt;Packages|000gt;Upgrade
...
[[_s3_sm_ssm_verify]]
==== System Set Manager |000gt;Packages|000gt;Verify
...
=== System Set Manager |000gt;Groups
...
These functions are limited to |000susemgr; Administrators and System Group Administrators.
...
=== System Set Manager |000gt;Channels


Manage channel associations through the following subtabs: 

[[_s4_sm_ssm_channels_base]]
==== System Set Manager|000gt;Channels|000gt;Base Channels
...
==== System Set Manager |000gt;Channels|000gt;Child Channels
...
=== System Set Manager |000gt;Configuration
...
==== System Set Manager|000gt;Configuration|000gt;Deploy Files


Use this subtab to distribute configuration files from your central repository on |000susemgr; to each of the selected systems. 
...
==== System Set Manager |000gt;Configuration|000gt;Compare Files


Use this subtab to validate configuration files on the selected systems against copies in your central repository on |000susemgr;. 
...
To compare the configuration files deployed on the systems with those in |000susemgr;, select the check box for each file to be validated.
Then click menu:Analyze Differences[Schedule File
     Comparison]
.
The comparisons for each system will not complete until each system checks in to |000susemgr;. When each comparison is complete, any differences between the files will be accessible from each system's events page. 
...
==== System Set Manager |000gt;Configuration|000gt;Subscribe to Channels
...
This tab is available only to |000susemgr; Administrators and Configuration Administrators. 
...
==== System Set Manager |000gt;Configuration|000gt;Unsubscribe from Channels
...
==== System Set Manager |000gt;Configuration|000gt;Enable Configuration
...
Then the systems will get subscribed to the required |000susemgr; tools channel and required [package]#rhncfg*#
 packages will get installed. 

[[_ref.webui.systems.ssm.provision]]
=== System Set Manager |000gt;Provisioning


Set the options for provisioning systems via the following subtabs. 

[[_ref.webui.systems.ssm.provision_autoinst]]
==== System Set Manager|000gt;Provisioning|000gt;Autoinstallation
...
, |000susemgr; will automatically pick a profile for each system so that the system's IP address will be in one of the IP ranges specified in the profile itself.
If such a profile cannot be found, |000susemgr; will look for an organization default profile and apply that instead. If no matching IP ranges nor organization default profiles can be found, no autoinstallation will be performed on the system.
...
|000susemgr; and its network must be properly configured to enable boot using PXE.
For more information on Cobbler and |000kickstart; templates, refer to <<_advanced.topics.cobbler>>
. 
...
If any of the systems connect to |000susemgr; via a proxy server, choose either the menu:Preserve Existing Configuration[]
...
==== System Set Manager |000gt;Provisioning|000gt;Tag Systems
...
==== System Set Manager |000gt;Provisioning|000gt;Rollback
...
==== System Set Manager |000gt;Provisioning|000gt;Remote Command
...
==== System Set Manager |000gt;Provisioning|000gt;Power Management Configuration
...
==== System Set Manager |000gt;Provisioning|000gt;Power Management Operation
...
=== System Set Manager |000gt;Audit
...
==== System Set Manager|000gt;Misc|000gt;Hardware
...
==== System Set Manager |000gt;Misc|000gt;Software
...
==== System Set Manager |000gt;Misc|000gt;Migrate
...
==== System Set Manager |000gt;Misc|000gt;Lock/Unlock
...
==== System Set Manager |000gt;Misc|000gt;Delete
...
==== System Set Manager |000gt;Misc|000gt;Reboot
...
The menu:Bootstrapp Minions[]
 page allows you to bootstrap Salt minions from the |000webui;. 
...
Users with the Activation Key Administrator role (including |000susemgr; Administrators) can generate activation keys in the |000susemgr; Web interface.
With such an activation key, register a |000sle; or Red Hat Enterprise Linux system, entitle the system to a |000susemgr; service level and subscribe the system to specific channels and system groups through the `rhnreg_ks` command line utility. 
...
....... menu:Key[] -- Either choose automatic generation by leaving this field blank or enter the key you want to generate in the menu:Key[] field. This string of characters can then be used with `rhnreg_ks` to register client systems with |000susemgr;. Refer to <<_s3_sm_system_keys_multiple>> for details. 
+
...
## 2011-03-24 - ke: bwiedemann 20110210: # wirklich alle? auch "'\ und
        0 bytes? Also see the quick starts! 2016-02-17 - moio: ," are not allowed |000lt;> (){} will
        get removed automatically #
 Do not insert commas or double quotes in the key.
All other characters are allowed, but `|000lt;> (){}` (this includes the space) will get removed automatically.
...
....... menu:Base Channels[] -- The primary channel for the key. This can be either the `|000susemgr; Default` channel, a |000suse; provided channel, or a custom base channel. 
+ 
Selecting `|000susemgr; Default` allows client systems to register with the |000suse;-provided default channel that corresponds with their installed version of |000sle;. You can also associate the key with a custom base channel.
If a system using this key is not compatible with the selected channel, it will fall back to the |000susemgr; default channel. 
....... menu:Add-on System Types[] -- The supplemental system types for the key, e.|000nbsp;g. Virtualization Host. All systems will receive these system types with the key. 
....... menu:Contact Method[] - Select how clients communicate with |000susemgr;. menu:Default[] (Pull) waits for the client to check in. With menu:Push via SSH[] and menu:Push via SSH tunnel[] the server contacts the client via SSH (with or without tunnel) and pushes updates and actions, etc. 
+ 
...
Separate keys with a comma at the command line with `rhnreg_ks` or in a |000kickstart; profile in the menu:Activation Keys[]
...
|000susemgr; Provisioning customers can create package profiles via the menu:System Details[]
 page. 
...
|000susemgr; customers may include completely customizable information about their systems. 
...
Third party hypervisors and hypervisor managers such as VMWare vCenter are called "`Virtual Host Managers`"
 (VHM) within |000susemgr;. These managers can manage one or multiple virtual hosts, which in turn may contain virtual guests.
|000susemgr; ships with a tool called `virtual-host-gatherer` that can connect to VHMs using their API, and request information about virtual hosts.
...
For more information, see the man page on your |000susemgr; server for ``virtual-host-gatherer``: 
