...
The left navigation bar is the menu to the |000susemgr; |000webui; from where you load the |000webui; pages.
...
Only |000susemgr; Administrators see the menu:Images[]
, menu:Users[]
, and menu:Admin[]
 entries. 
...
This section summarizes all of the categories and primary pages (those linked from the left navigation bar) within the |000susemgr; Web interface (|000webui;). It does not list the many subpages, tabs and subtabs accessible from the individual pages.
...
Generate an activation key for a |000susemgr;-entitled system.
...
Display and modify various aspects of autoinstallation profiles (Kickstart and |000ay;) used in provisioning systems. 
* {empty}
+
...
.menu:Software[]
View and manage the available |000susemgr; channels and the files they contain. 
...
.menu:Configuration[]
Keep track of and manage configuration channels, actions, individual configuration files, and systems with |000susemgr;-managed configuration files. 
...
.menu:Systems[]
List the systems that have |000susemgr;-managed configuration files. 
...
.menu:Admin[]
-- Use the Setup Wizard to configure |000susemgr;. List, create, and manage one or more |000susemgr; organizations.
The |000susemgr; administrator can assign channel entitlements, create and assign administrators for each organization, and other tasks. 
...
.menu:Users[]
List all users known by |000susemgr;, across all organizations.
Click individual user names to change administrative privileges of the user. 
+

[NOTE]
====
Users created for organization administration can only be configured by the organization administrator, _not_ the |000susemgr; administrator. 
====
* {empty}
+

.menu:Manager Configuration[]
Make General configuration changes to the |000susemgr; server, including Proxy settings, Certificate configuration, Bootstrap Script configuration, Organization changes, and Restart the |000susemgr; server. 
* {empty}
+
...
View the status of the various tasks of the |000susemgr; task engine. 
* {empty}
+

.menu:Show Tomcat Logs[]
Display the log entries of the Tomcat server, on which the |000susemgr; server is running. 
...
Throughout |000susemgr; you will see three patch (errata) alert icons. 

image:fa-shield.svg[Security Alert]
 |000mdash; represents a security alert. 

image:fa-bug.svg[Bug Fix Alert]
 |000mdash; represents a bug fix alert. 

image:spacewalk-icon-enhancement.svg[Enhancement Alert]
 |000mdash; represents an enhancement alert. 
...
In the top bar, |000susemgr; offers a search functionality for Packages, Patches (Errata), Documentation, and Systems.
...
If you misspell a word during your search query, the |000susemgr; search engine performs approximate string (or fuzzy string) matching, returning results that may be similar in spelling to your misspelled queries. 

For example, if you want to search for a certain development system called `test-1.example.com` that is registered with |000susemgr;, but you misspell your query ``tset``, the `test-1.example.com` system still appears in the search results. 

[NOTE]
====
If you add a distribution or register a system with a |000susemgr; server, it may take several minutes for it to be indexed and appear in search results. 
====
...
[NOTE]
====
Performing operations on large lists|000mdash;such as removing RPM packages from the database with the |000susemgr; Web interface|000mdash;may take some time and the system may become unresponsive or signal "`Internal Server
    Error 500`"
.
