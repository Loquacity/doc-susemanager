[[_ref.webui]]
= Web Interface |000mdash; Navigation and Home
:doctype: book
:sectnums:
:toc: left
:icons: font
...
[abstract]
--
This is a chapter about the layout of the Web Interface to |000susemgr;, basic usage information, and the menu:Home[]
 menu. 
--
...
Entering the |000susemgr; URL in a browser takes you to the menu:Sign In[]
 screen.
...
After logging into the Web interface of |000susemgr;, the first page to appear is the menu:Overview[]
...
[NOTE]
====
If you are new to the |000susemgr; Web interface, read <<_ref.webui.intro>>
...
* The menu:Tasks[] section lists the most common tasks an administrator performs via the Web interface. Click any link to reach the page within |000susemgr; that allows you to accomplish that task. 
+

.
image::home_overview_tasks_pane.png[]
* If any systems have not been checking in to |000susemgr;, they are listed under menu:Inactive Systems[] to the right. Highlighting them in this way allows an administrator to quickly select those systems for troubleshooting. 
+
...
* The menu:Recently Registered Systems[] section lists the systems added to the |000susemgr; in the past 30 days. Click a system's name to see its menu:System Details[] page. Click the link below this section to menu:View All Recently Registered Systems[] on one page. 
+
...
If you change your |000susemgr; password, for security reasons you will not see the new password while you enter it.
...
The email address listed in the menu:Your Account[]
 page is the address to which |000susemgr; sends email notifications if you select to receive patch alerts or daily summaries for your systems on the menu:Your Preferences[]
 page. 
...
View or enter external system or API credentials associated with your |000susemgr; account, for example your |000suse; Studio credentials. 
...
The menu:Account Deactivation[]
 page provides a means to cancel your |000susemgr; service.
To do so, click the menu:Deactivate
    Account[]
 button.
The Web interface returns you to the login screen.
If you attempt to log back in, an error message advises you to contact the |000susemgr; administrator for your organization. 

.
image::home_user_account_account_deactivation.png[]


.|000susemgr; Administrator Account
[NOTE]
====
If you are the only |000susemgr; Administrator for your organization, you are unable to deactivate your account. 
====
...
The menu:Home[Your
   Preferences]
 page allows you to configure |000susemgr; options. 
...
Add more systems either individually on the menu:System Details[]
 ...
 interface.
Note that |000susemgr; sends these summaries only to verified email addresses.
To disable all messages, simply deselect this check box. 
====
* menu:|000susemgr; List Page Size[] -- Maximum number of items that appear in a list on a single page. If more items are in the list, clicking the menu:Next[] button displays the next group of items. This preference applies to system lists, patch lists, package lists, and so on. 
* menu:|000quot;Overview|000quot; Start Page[] -- Select the information sections that are displayed on the menu:Overview[] page of the menu:Home[] menu. Check the box to the left of the information section you would like to include. 
...
On the menu:Home[Locale
   Preferences]
 page set your |000susemgr; interface to your local time by selecting the appropriate menu:Time Zone[]
...
[NOTE]
====
Keep in mind states are applied according to the following order of hierarchy within |000susemgr;: `Organization` > `Group` > ``Single System``. 
====
